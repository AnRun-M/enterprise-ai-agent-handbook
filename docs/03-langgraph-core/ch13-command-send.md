...
| Node 返回部分更新、不决定下一步（第 10 章） | 在本章讨论的场景中，Command 是 **Node 可返回的一种 Runtime 控制结果**——"返回更新 + 路由意图"，不是节点获得调度权 |
...
**等价性必须收窄**：在**单图 Node update + goto 场景**中，Command 与 Node Update + Conditional Edge **可以表达相近的"更新后导航"意图**——但**不宣称全面等价**：表达位置不同（图外 callable 定义 vs 节点返回结果）、耦合方式不同（更新与导航绑定 vs 分离）、扩展能力不同（Command 的 parent graph / resume / Tool return 等场景不属于该对照）。**当前仓库未实现等价性测试**（13.9 未验证清单）。表达方式变化，解释权不变——真正执行跳转的仍然是 Graph Runtime。
...
| work item：可执行步骤 | Node（第 10 章） | Send 描述、Graph Runtime 实例化的 work items |
...
**边界不变**：Send **不自己执行节点、不自己创建线程**——它表达 fan-out，实例化与调度在 Graph Runtime；Command 表达路由意图但不自己跳转——**调度执行始终在 Scheduler / Graph Runtime**（第 6 章 6.7：Scheduler 决定"把控制权交给谁"，不制定规则）。**"Send 表达 fan-out"不自动保证**：并发度、调度顺序、稳定结果顺序、线程安全、重试、delivery semantics 与 fan-in 合并确定性（13.9 未验证清单）。
...
1. **Command 是新的调度器**——在本章场景中，Command 是 Node 可返回的一种 Runtime 控制结果（State Update + goto 路由组合）；解释与调度仍在 Graph Runtime
...
| Q7 | Command / Send 与 ch06 Scheduler / Dispatch 如何对应？ | Send 由 conditional routing callable 返回、Graph Runtime 实例化并调度（work item 调度属 Scheduler）；Command 表达路由意图，不自己跳转——调度语义原样成立 |
...
- [ ] 能说清 Send 的产生链路（Node 产数据 → conditional routing callable 返回 Send descriptors → Graph Runtime 解释、实例化并调度）与独立输入语义（目标节点 + 专属输入；同一目标可实例化多次）

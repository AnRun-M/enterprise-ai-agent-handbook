...
| Part 3：LangGraph Core（StateGraph / Node / Edge / Conditional Edge / Reducer / Command / Send / Checkpoint / Interrupt / Stream / Subgraph） | 图运行时、恢复、流式 | `examples/basic_langgraph` | Graph path、Checkpoint recovery | 进行中（示例与等价 Demo 已完成；第 8/9/10/11/12/13/14 章最终完成，第 15 章正文初稿） | v0.4.0 |
...
| 第 15 章：Interrupt——暂停与人工介入 | 可恢复执行点暂停、控制权交还应用或人工参与者、同一 thread 持久化状态恢复（可携带人工输入或控制结果）、≠ END / ≠ 普通异常 / ≠ 完整 HITL、Checkpoint 提供承载 + Interrupt 提供协议、ch01 Human Stop 暂停态对应、T07 人工审批挂载点 | `examples/basic_langgraph`（README 第 18 节：HITL 属 v0.4.0 / v0.6.0）、`examples/checkpoint_hitl`（预留） | 无（Demo 未使用，如实标注；证据为第 1 章声明 / 官方核验记录） | 实现完成 / 待架构审查 | v0.4.0 |

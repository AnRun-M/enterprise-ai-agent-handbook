...
| 第 8 章：为什么是图——为什么 Runtime 可以用 Graph 表达 | 循环可显式 / 连接可声明 / 边界可挂载、图带来与没带来、图不引入新 Runtime 理论、适用边界 | `examples/basic_langgraph`（README 16/17/18 节） | 双 Runtime 行为等价（`test_direct_equivalence_with_manual`） | 进行中（正文初稿，待 Architecture Review） | v0.4.0 |

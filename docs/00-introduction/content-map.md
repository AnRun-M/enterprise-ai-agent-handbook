...
| 第 6 章：Runtime Scheduler & Orchestration | Loop vs Scheduler、State Transition 调度、Control Plane 编排、Scheduler/Policy/LLM 边界、Runtime 可替换 | `examples/manual_agent_loop`、`examples/basic_langgraph` | 路由纯函数、等价对照、max_iterations | 最终完成 | v0.3.0 |
| Part 2：Agent Runtime（LLM 与 Agent / Agent Loop / Runtime / State / Tool Registry / Prompt Builder / Memory 与 Context / 手写 Runtime） | Runtime 概念、State 生命周期 | `examples/manual_agent_loop` | State reducer、Tool adapter | 进行中（Chapter 02-06 已完成，Chapter 07 规划中） | v0.3.0 |
| 第 7 章：Memory、Context 与 Context Management | Memory 与 State/Context/Checkpoint 边界、Context Window、History、Compression/Trimming/Summarization、Injection、生命周期与事实源 | `examples/manual_agent_loop`、`examples/basic_langgraph` | 等价对照 | 规划 | v0.3.0 |

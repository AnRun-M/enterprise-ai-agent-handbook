...
| Part 2：Agent Runtime（Execution State / Model Context / Prompt Builder / Tool Registry / Runtime Scheduler & Orchestration / Memory、Context 与 Context Management） | 框架无关 Runtime 语义、状态与上下文、模型输入组装、能力管理、执行编排、上下文生命周期 | `examples/manual_agent_loop`、`examples/basic_langgraph` | State transition、路由决策、双 Runtime 行为等价 | 进行中（Chapter 02-06 已完成，Chapter 07 规划中） | v0.3.0 |
| 第 7 章：Memory、Context 与 Context Management | Memory 与 State/Context/Checkpoint 边界、Context Window、History、Compression/Trimming/Summarization、Injection、生命周期与事实源 | 待规划 | 待规划 | 实现完成 / 待架构审查 | v0.3.0 |

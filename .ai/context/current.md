...
阶段：**Part 03（LangGraph Core Runtime Execution Model）完成**（Chapter 08-17 全部最终完成，2026-08-07，Part 03 Release Audit 通过，TASK-0025）；**v0.4.0 完成**；**Part 04 进行中**（v0.5.0：Text-to-SQL 重构）——前置章 Chapter 18 最终完成（PR #51）；**T01-T12 Execution Planning 完成**（TASK-0029，PR #53）；**T05 implementation task 完成**（TASK-0030，PR #56，首个 implementation task）；**Wave 1 进行中**（**T01（TASK-0032）：Gate A 通过（PR #59 合并）→ Gate B/C APPROVED → Gate D 完成 → Task Merge Gate Review 修正中**（feature/t01-input-normalization，normalized_question / 纯函数+Node adapter / field ownership + transition authority：success 不覆盖 shared lifecycle、failure 仅 RUNNING→FAILED / 第 19 章 T01 部分 draft，等待最终确认）；**T03（TASK-0033）：Gate A complete，Implementation not started**）；v0.5.0 尚未宣布完成；`v0.3.0` 全部完成（Chapter 01-07 最终完成，Part 02 收官）
...
1. **T01 input normalization（TASK-0032，feature/t01-input-normalization，Task Merge Gate Review 修正中）**：Gate A 通过（PR #59 合并 `eb9d324`）；Gate B 实现（`examples/text2sql_state/state.py` Text2SQLState 最小契约 + `normalization.py` `normalize_question` 纯函数 + `normalize_node.py` `normalize_input_node` Node adapter）；Gate C 测试通过（pytest / ruff / mkdocs --strict 全绿）；Gate B/C 最终复审 **APPROVED**；**Gate D 完成**——`docs/04-text2sql/ch19-input-normalization-intent.md`（draft，T01 部分；T02 pending 标注；19.1-19.10；四列制证据；不写死 pytest 数量）+ index/mkdocs 导航，content-map 未动（无逐章行惯例）。**Task Merge Gate Review（两轮）field ownership 修正**：normalized_question = T01-owned derived field；status / failure_reason = shared lifecycle fields（"Field write capability ≠ field ownership."）——success 只写 normalized_question（不重置任意 FAILED lifecycle）；failure 无条件清 normalized_question，**仅 RUNNING 时**追加 status=FAILED + failure_reason（**transition-scoped："Shared field ownership can be transition-scoped, not field-wide."**——已 FAILED 的 cause 不替换、SUCCESS/MAX_ITERATIONS_REACHED 不被空输入改成 FAILED）；错误 success stale-failure regression 已删，新增 lifecycle non-overwrite + failure-cause-preservation regression；retry/new-request boundary 明确；Chapter 19 19.5/19.6/19.7/19.8 按 ownership + transition authority 重写；merge simulation 收窄为 dict overwrite 模拟（非 Graph integration evidence）。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**；等待 Task Merge Gate 最终复审；Chapter 19 未标 completed。**T03（TASK-0033）：Gate A complete，Implementation not started（本轮未实现 T03）。**

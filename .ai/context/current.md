...
1. **T01 input normalization（TASK-0032，feature/t01-input-normalization，Gate D 完成待 Task Merge Review）**：Gate A 通过（PR #59 合并 `eb9d324`）；Gate B 实现（`examples/text2sql_state/state.py` Text2SQLState 最小契约 + `normalization.py` `normalize_question` 纯函数 + `normalize_node.py` `normalize_input_node` Node adapter）；Gate C 测试通过（pytest / ruff / mkdocs --strict 全绿）；Gate B/C 最终复审 **APPROVED**（两轮修正：完整 outcome update / merge regression 双向 / evidence 命名收窄 / whitespace policy 边界）；**Gate D 完成**——`docs/04-text2sql/ch19-input-normalization-intent.md`（draft，T01 部分；T02 pending 标注；19.1-19.10；四列制证据；不写死 pytest 数量）+ index/mkdocs 导航，content-map 未动（无逐章行惯例）。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（T02 尚未进 main，不宣称 T01→T02 integration verified）；等待 Task Merge Gate 最终 Review；Chapter 19 未标 completed。**T03（TASK-0033）：Gate A complete，Implementation not started（本轮未实现 T03）。**

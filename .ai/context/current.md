...
阶段：**Part 03（LangGraph Core Runtime Execution Model）完成**（Chapter 08-17 全部最终完成，2026-08-07，Part 03 Release Audit 通过，TASK-0025）；**v0.4.0 完成**；**Part 04 进行中**（v0.5.0：Text-to-SQL 重构）——前置章 Chapter 18 最终完成（PR #51）；**T01-T12 Execution Planning 完成**（TASK-0029，PR #53）；**T05 implementation task 完成**（TASK-0030，PR #56，首个 implementation task）；**Wave 1 三个首推 implementation 全部达到 Task-level complete + Integration deferred（T01 ✅ / T03 ✅ / T05 ✅）**（**T01（TASK-0032）：completed**（PR #60 合并 `bfd1880`，Gate A/B/C/D/Task Merge 全通过；Evidence = Contract-level verified；Integration = deferred 等 T02——Gate E 属 Milestone Integration Closure 非阻塞；第 19 章 T01 部分 draft，T02 pending 未标 completed）；**T03（TASK-0033）：completed**（PR #62 合并 `4cb6b44`，Gate A/B/C/D/Task Merge 全通过；Evidence = Contract-level verified；Integration = deferred 等 T02/T04——Gate E 属 Milestone Integration Closure 非阻塞；第 20 章 **T03 content complete**，标 draft 留 Part 04 release closure 统一收敛状态标注）；**T05（TASK-0030）：completed**（PR #56 合并 `b2572e6`））；v0.5.0 尚未宣布完成；`v0.3.0` 全部完成（Chapter 01-07 最终完成，Part 02 收官）
...
1. **T02 intent & semantic parsing（TASK-0034，planning/t02-intent-semantic-contract，Gate A 进行中）**：Wave 1 三个首推 implementation（T01/T03/T05）已全部 Task-level complete + Integration deferred；下一轮按 TASK-0029 Recommended Implementation Waves + Integration Dependency Matrix 选择——**T02 为最自然候选**（unblock T01→T03 Integration Closure；Implementation dependency ≠ Runtime pipeline order，不因 T03 后是 T04 就自动启动 T04）。Gate A 草案（TASK-0034）：固定主线（**结构化语义解释与意图**，非"结构化语义事实"；"Semantic interpretation is structured inference, not authoritative fact."；不生成 SQL / 不读权威 metadata / 不伪装事实）；T01/T02 边界（"Normalization changes representation; semantic parsing interprets meaning."）；T02/T03 边界（"Semantic interpretation may identify what facts are needed; it must not manufacture authoritative facts."）；Contract 方案比较（A 多字段 / B 独立对象 / C typed contract + 单 State channel × 11 维度）→ **方案 C：IntentResult**（TASK-0029 Proposed 命名，T02-owned derived state，单 channel）；Ambiguity 双层（interpretation vs authoritative-source，不混字段）；Outcome taxonomy 四态（COMPLETE / PARTIAL / AMBIGUOUS / UNSUPPORTED；不机械复制 T03 五态）。**第一轮 Review 修正（已应用）**：① 四态均返回**完整 IntentResult**，UNSUPPORTED 不以 None 表达（"Expected semantic outcomes always produce an IntentResult."；None 仅限无合法 result 的 failure path，留 Gate B failure-boundary Review）② COMPLETE/PARTIAL 基于 **required semantics**（"Optional semantic field absence is not automatically partial interpretation."；not-applicable vs required-but-unresolved 可区分）③ T02→T03 升级**三层**（IntentResult → source-agnostic retrieval requirements → source-specific adapter → T03 RetrievalCriteria；"Semantic interpretation ≠ retrieval requirement ≠ source-specific retrieval criteria."；RetrievalCriteria 保持 fixture，fake source key 不进 IntentResult，不强制 DTO、Gate B 不得删逻辑层）④ ambiguity **暴露不消歧**（"T02 exposes ambiguity; it does not unilaterally resolve it."——后续 policy 决定 clarification / human input / domain rule / T03 facts 验证）⑤ model failure **不属于 semantic taxonomy**（retryability 未冻结，不默认全 transient）⑥ consumed-contract 边界（pure parser 收 str，Node adapter 保证契约）。**第二轮 Review 修正（已应用）**：Outcome→Retrieval Requirement **Eligibility 冻结**（COMPLETE/PARTIAL/AMBIGUOUS 各自 eligibility；UNSUPPORTED 默认不产生普通 downstream retrieval requirements，未来需要单独设计）；**Retrieval Requirement ≠ Routing**（"Retrieval requirement is data, not routing."——T02 不调用/route/retry T03、不 clarification routing、不 terminate graph，属 application control flow/Node/Edge/Command/Runtime）；AMBIGUOUS 可产生 candidate-scoped requirements 但**不自动进 T03**（policy 四选项：clarification / human input / domain rule / T03-assisted disambiguation；ambiguous outcome ≠ routing decision）；**resolved / candidate / unresolved / not-applicable 四语义状态可区分**（语义类别表述为 semantic interpretations (resolved or candidate)）；Integration edge 细化（T01→IntentResult / IntentResult→requirements / requirements→RetrievalCriteria / RetrievalCriteria→T03 RetrievalResult / RetrievalResult→T04 / compiled graph / real source——**data edge ≠ routing edge**）；time/filter/entity 只出 semantic token 不制造外部事实；T02→T04 边界（IntentResult 不只服务 T03）；fake semantic parser 优先不接真实 LLM；**Integration evidence edge-scoped（"Integration evidence is edge-scoped, not task-wide."——T01→T02 / T02 result→retrieval requirements / retrieval requirements→T03 RetrievalCriteria / T03→T04 / compiled graph / real source 各自 deferred/closed，不用一句 T02→T03 closed 掩盖 adapter edge）**。**Gate A 最终状态（2026-08-14）：Architecture Review = APPROVED（final consistency cleanup 完成：retrieval criteria 旧口径 / resolved-candidate 术语 / RetrievalCriteria final boundary / adapter 三层表述已统一，residual scan 无残留）；Status 仍 in_progress；等待最终 Merge 确认；不得进入 Gate B。** **Gate A 合并（2026-08-14）：PR #64 squash merge `c7897b2` 进 main；planning/t02-intent-semantic-contract 本地/远程分支已删、`git fetch --prune`、main == origin/main（8 项合并后验证全过：PR #64 = MERGED / main == origin/main / working tree clean / TASK-0034 = in_progress / Gate A = APPROVED / 未存在 T02 implementation / Part 04 = in progress / v0.5.0 = incomplete）→ Gate A 正式完成。** **Gate B/C（2026-08-14，feature/t02-intent-semantic-parsing，PR #65，等待 Architecture + Implementation Review，不得 Merge）**：实现 `semantic_types.py`（IntentOutcome 四态 / SemanticState 四语义状态 / SemanticValue / IntentResult / RetrievalRequirement + RetrievalPurpose）/ `semantic_parser.py`（FakeSemanticParser，deterministic 不接 LLM）/ `semantic_node.py`（只写 intent_result，不写 status/failure_reason，不调用/路由 T03，normalized=None → ValueError）/ `retrieval_adapter.py`（requirements → T03 RetrievalCriteria fixture，空 requirements → None）/ `state.py`（intent_result 单 channel）。**非法组合自然防止**：outcome 派生（COMPLETE 含 required-unresolved 不可表达）/ AMBIGUOUS_CANDIDATES 无 resolved 值（静默选择不可表达）/ UNSUPPORTED requirements 恒空 + adapter 返 None（自动生成普通 RetrievalCriteria 不可表达）；其余不变式构造边界 fail-fast（同 T03 source-boundary 哲学）。**failure-boundary 决策**：normalized=None → ValueError，Node 不写 None（合法路径整体 overwrite）。Gate C：4 测试文件、`pytest tests/text2sql_state` 224 passed；integration evidence edge-scoped（data edge ≠ routing edge）；real source / compiled graph 仍 deferred。** **Gate B/C Architecture + Implementation Review 修正（2026-08-14，PR #65 追加 commit，等待最终复审；不得 Merge / 不得进入 Gate D）**：① SemanticValue `__post_init__` 运行时校验 state 类型（TypeError，malformed state 不得落入 else 分支）② IntentResult 七 category 运行时校验 SemanticValue（TypeError，str/None/dict 不得延迟到派生属性才 AttributeError）③ unsupported_reason invariant 移入 `__post_init__`（None 或 non-empty trimmed；factory 收敛为 convenience）④ bypass tests（direct constructor 与 factory 同一 contract）⑤ RetrievalRequirement `semantic_ref` → **`semantic_refs: tuple[str, ...]`**（候选结构不降级为 join 字符串："Structured candidate semantics must remain structured across contract boundaries."）⑥ RetrievalRequirement shape invariant（VERIFY 恰好 1 ref / AMBIGUOUS ≥2 distinct refs / COMPLETE refs 空；category/purpose 运行时类型校验）⑦ adapter 同步（VERIFY 唯一 ref 映射；AMBIGUOUS 按 category 映射且候选保留在 requirement；COMPLETE 按 category 不伪造 ref）⑧ candidate-structure tests（含逗号候选结构不丢 / duplicate 仍拒 / adapter 不改 requirements）⑨ **SemanticParser Protocol**（`parse_intent_node` 依赖语义契约而非 FakeSemanticParser fake implementation）⑩ failure boundary 保持（None → ValueError → no State update，不写 intent_result=None，exception path ≠ UNSUPPORTED）。验证：`pytest tests/text2sql_state` **242 passed**；`ruff check .`（0.16.3，CI 同版本）**All checks passed**；`mkdocs build --strict` 通过；`git diff --check` 干净。** **Gate B/C 最终复审修正（2026-08-14，PR #65 追加 commit，等待最终确认；不得 Merge / 不得进入 Gate D）——闭合 runtime payload contract**：① `SemanticValue.resolved` RESOLVED 先 `isinstance(str)` → TypeError 再 non-empty/trimmed → ValueError（不得 `.strip()` AttributeError）② `candidates` 容器必须 tuple → TypeError（frozen dataclass 不接受 mutable list）③ candidate 元素先 `isinstance(str)` → TypeError 再 domain 校验 ④ `semantic_refs` 容器必须 tuple → TypeError ⑤ ref 元素先 `isinstance(str)` → TypeError 再 non-empty/trimmed ⑥ `unsupported_reason` 非 None 且非 str → TypeError（不得 int.strip()）。**固定原则新增**："A runtime contract must validate both the discriminant and its payload shape."（保留 "Static type annotation ≠ runtime contract validation."）。**Error taxonomy 统一**：TypeError = runtime 类型/容器错误；ValueError = 类型正确但 domain value 不合法。direct-constructor tests +8（含 immutability regression：合法 candidates/semantic_refs 均为 tuple）。只改 `semantic_types.py` + `test_semantic_contract.py`；验证：`pytest tests/text2sql_state` **250 passed**（全量除 langgraph 281）/ ruff All checks passed / mkdocs build --strict 通过 / git diff --check 干净。** **Gate B/C 最终 Architecture Review 修正（2026-08-14，PR #65 追加 commit，等待最终确认；不得 Merge / 不得进入 Gate D）——闭合 resolved interpretation vs authoritative grounding 缺口**：① 固定原则 "Semantic resolution ≠ authoritative grounding completeness."（time RESOLVED("yesterday") ≠ timezone/calendar/freshness 已齐全）② 禁止把 resolved time 改成 unresolved 触发 retrieval（语义错误）③ 新增 `RetrievalPurpose.GROUND_EXECUTION_CONTEXT`（interpretation 已 resolved 但执行需 authoritative context）④ shape invariant：恰好 1 个 non-empty trimmed ref ⑤ 派生：TIME_RANGE+RESOLVED → GROUND；aggregation/query intent 不自动 grounding；VERIFY / AMBIGUOUS / REQUIRED_UNRESOLVED 逻辑保持 ⑥ adapter：TIME_RANGE+GROUND → "business_calendar"（fake vocabulary 不进 IntentResult）⑦ COMPLETE_INTERPRETATION（interpretation 缺失）vs GROUND_EXECUTION_CONTEXT（interpretation 已解析待 grounding）不混淆。测试 +5（outcome 不因 grounding need 变 PARTIAL / requirements 双含 VERIFY+GROUND / adapter business_calendar / NOT_APPLICABLE 无 grounding / unresolved 不误走 GROUND / GROUND shape）。**Integration edge**：IntentResult→grounding requirement = contract-level；grounding requirement→business_calendar criteria = adapter-level；business_calendar→real fact = **deferred**（不改 T03 fixture；T03 诚实 NOT_FOUND → PARTIAL edge 测试）；不宣称 time grounding closed；compiled graph / real source 仍 deferred。验证：`pytest tests/text2sql_state` **255 passed**（全量除 langgraph 286）/ ruff All checks passed / mkdocs build --strict 通过 / git diff --check 干净。** **Gate B/C 最终 cleanup（2026-08-14，PR #65 追加 commit，等待最终确认；不得 Merge / 不得进入 Gate D）——Category × Purpose compatibility matrix**：`RetrievalRequirement.__post_init__` 冻结 `_CATEGORY_PURPOSE_MATRIX`（cross-field invariant）：VERIFY 仅 METRIC/DIMENSION/ENTITY/FILTER；GROUND 当前仅 TIME_RANGE；RESOLVE_AMBIGUITY 保持 source-agnostic（不因 fake adapter 只支持 METRIC 而限制）；COMPLETE_INTERPRETATION 保持所有可 REQUIRED_UNRESOLVED 类别。非法组合 construction boundary → ValueError（TIME_RANGE/AGGREGATION/QUERY + VERIFY；METRIC/DIMENSION/ENTITY/FILTER/AGGREGATION/QUERY + GROUND；TIME_RANGE+GROUND 合法）。**domain semantic validity ≠ source mapping availability**（METRIC+VERIFY("unknown_metric") 构造成功，adapter 抛 ValueError）。`_FACT_GROUNDED_CATEGORIES`（str 集合）→ `_DEFINITION_GROUNDED_CATEGORIES`（SemanticCategory Enum 集合），派生直接 `category in ...`。固定原则新增："Payload shape validity ≠ semantic combination validity." / "Domain contracts must validate cross-field invariants, not only each field independently."。只改 `semantic_types.py` + `test_semantic_contract.py` + `test_retrieval_adapter.py`；验证：`pytest tests/text2sql_state` **262 passed**（全量除 langgraph 293）/ ruff All checks passed / mkdocs build --strict 通过 / git diff --check 干净。**
2. **T01 input normalization（TASK-0032）**：**completed**（PR #60 squash merge `bfd1880`，分支已删）——Gate A（PR #59 `eb9d324`）→ Gate B/C（两轮 Review 修正 + 最终复审 APPROVED）→ Gate D（`docs/04-text2sql/ch19-input-normalization-intent.md`，draft，T01 部分）→ Task Merge Gate（两轮修正：field ownership → transition-scoped lifecycle authority）；`examples/text2sql_state/`：`state.py`（Text2SQLState）/ `normalization.py`（纯函数）/ `normalize_node.py`（Node adapter，success 只写 normalized_question、failure 仅 RUNNING→FAILED）+ 20 项测试。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（Gate E 属 Milestone Integration Closure，等 T02 进 main，非 T01 完成阻塞）；Chapter 19 = 不标 completed（T02 pending）；Part 04 = 进行中；v0.5.0 = 未完成。
3. **T03 metadata & business rule retrieval（TASK-0033）**：**completed**（PR #62 squash merge `4cb6b44`，分支已删）——Gate A（PR #59 `eb9d324`）→ Gate B/C（三轮 Review 修正 + 最终复审 APPROVED）→ Gate D（`docs/04-text2sql/ch20-metadata-business-rule-retrieval.md`，draft，**T03 content complete**；T04 / Context Builder 仅接口位置）→ Task Merge Gate（三轮修正：方案 A fact-level binding → identity/evidence 分离 → 最终 code-contract cleanup，最终确认 APPROVED）；`examples/text2sql_state/`：`retrieval_types.py`（RetrievalOutcome 五态 / RetrievalReference / MaterializedFact / MaterializedFacts / RetrievalResult / RetrievalCriteria（Proposed fixture））/ `metadata_source.py`（CatalogEntry entry_id + fake authoritative source）/ `retrieval.py`（MetadataRetriever，fact_id = source:entry_id，受限 grammar）/ `retrieval_node.py`（只写 retrieval_result，Outcome ≠ Agent lifecycle）/ `state.py`（retrieval_result 字段，教学规模选择）+ 55 项 retrieval 测试 + node 测试。**状态快照**：Evidence = Contract-level verified（provenance binding / fact identity uniqueness / fact-evidence separation / source-qualified identity string encoding 无歧义；global uniqueness / cross-system identity / production URI scheme / production lineage schema 未验证）；Integration = **deferred**（T02 未实现 / T03→T04 未真实集成 / compiled Graph Runtime path 未验证 / 真实 External Source 未接入；Gate E 属 Milestone Integration Closure，非 T03 完成阻塞）；Chapter 20 = draft（T03 content complete，留 Part 04 release closure 统一收敛状态标注）；Part 04 = 进行中；v0.5.0 = 未完成。
4. **T05 implementation task（SQL 静态校验，首个 implementation task，TASK-0030）**：**completed**（PR #56 合并 `b2572e6`）——Gate A（PR #55）→ Gate B/C → Gate D（Ch22 T05 部分）→ Task Merge Gate 全流程通过；`examples/text2sql_state/validation.py`（`_RULE_CHECKS` 无序 registry + RULE_ORDER 唯一顺序事实源 + `_statements` total contract + 超长 LIMIT 归入 limit_exceeds）；`docs/04-text2sql/ch22-sql-validation-repair-loop.md`（T05 可证实部分，T07 接口位置预留）。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（Gate E 等 T06/T07 进 main 后关闭）；Chapter 22 = 不标最终完成（T07 未实现）；Part 04 = 进行中；v0.5.0 = 未完成。
5. **Chapter 17 Ending maintenance（TASK-0027）已完成**：Chapter 17 Ending 句已修正为与冻结决策一致（"下一部分将进入 StateGraph 构图与 Graph Runtime 执行模型——图如何被组装、compile 如何将其转换为可执行 Runtime、invoke/stream 如何驱动执行，而不是重新定义这些运行时概念"），独立 PR 合并完成（commit b93f9a5）。
6. **Chapter 18 正文初稿（2026-08-08，TASK-0028，本任务；Part 04 前置章）**：
...
7. **未决项**：① Mermaid 计数漂移（历史 TASK / PR 记录与实际围栏数不一致；正文围栏数是事实源，后续任务不预先承诺固定数量）② v1.0.0 章节数目标对账（TASK-0014 未决项延续）③ 官方 URL 发布前复核（TASK-0014 未决项延续）④ RetryPolicy 机制归属（TASK-0014 未决项延续）
8. 补 tests/ 其余测试目标（State reducer、Tool adapter、Graph path、Checkpoint recovery）
9. 核验 Anthropic《Building effective agents》与 OpenAI practical guide 的官方 URL（第 0 章 TODO）
10. 选择许可证
11. **Future Task：LangChain Scope Planning**（不立即执行，仅记录方向；本条目已按 2026-08-05 规划合并补全，不新增第二份）：
...
12. **Future Backlog：Agent Workflow Patterns Scope Planning**（TASK-0031，proposed 登记，v0.6.0+，不立即执行）：

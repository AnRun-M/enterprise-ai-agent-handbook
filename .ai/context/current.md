...
2. **T03 metadata & business rule retrieval（TASK-0033，feature/t03-metadata-retrieval，Gate B/C 待 Review）**：Gate A 通过（PR #59 合并 `eb9d324`）；Gate B 实现（`retrieval_types.py` RetrievalOutcome 五态 / RetrievalReference / MaterializedFacts / RetrievalResult / RetrievalCriteria（Proposed fixture）+ `metadata_source.py` InMemoryMetadataSource fake 权威源（exists/missing/ambiguous/unavailable/partial）+ `retrieval.py` MetadataRetriever（确定性聚合 UNAVAILABLE > AMBIGUOUS > NOT_FOUND > PARTIAL > COMPLETE）+ `retrieval_node.py` retrieve_metadata_node（只写 retrieval_result，**不触碰 shared lifecycle——Outcome ≠ Agent lifecycle**）+ `state.py` retrieval_result 字段（教学规模选择））；Gate C 测试通过（pytest / ruff / mkdocs --strict 全绿，25 项 T03 测试：五态 / provenance / 分离 / determinism / 无污染 / 无静默兜底 / 无 LLM 事实 / 无路由意图 / lifecycle 边界）。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（T02 未实现 / T04 未集成 / 真实 External Source 未接入）；等待 Gate B/C Architecture + Implementation Review 通过后再进入 Gate D——**Review 修正已应用（等待最终确认）**：order-stability（full scan + priority aggregation，无 early return；permutation-invariant 输出）/ UNAVAILABLE payload 策略 A（保留成功 facts）/ empty criteria = contract violation（ValueError）/ **CatalogEntry 构造级运行时校验**（`__post_init__` 拒绝非 CatalogEntryKind——"Static type annotation ≠ runtime contract validation."）/ **source index/entry identity 校验**（构造即拒绝 key mismatch——"Provenance correctness starts at source construction"；read-only snapshot 表述收窄 + snapshot isolation 测试）/ duplicate-key 去重。

...
阶段：**Part 03（LangGraph Core Runtime Execution Model）完成**（Chapter 08-17 全部最终完成，2026-08-07，Part 03 Release Audit 通过，TASK-0025）；**v0.4.0 完成**；**Part 04 进行中**（v0.5.0：Text-to-SQL 重构）——前置章 Chapter 18 最终完成（PR #51）；**T01-T12 Execution Planning 完成**（TASK-0029，PR #53）；**T05 implementation task 完成**（TASK-0030，PR #56，首个 implementation task）；**Wave 1 进行中**（**T01（TASK-0032）：completed**（PR #60 合并 `bfd1880`，Gate A/B/C/D/Task Merge 全通过；Evidence = Contract-level verified；Integration = deferred 等 T02——Gate E 属 Milestone Integration Closure 非阻塞；第 19 章 T01 部分 draft，T02 pending 未标 completed）；**T03（TASK-0033）：Gate A 通过（PR #59）→ Gate B/C APPROVED → Gate D 完成**（feature/t03-metadata-retrieval，RetrievalResult 三层 / fake authoritative source / Retriever / Node 不触碰 lifecycle / 第 20 章 T03 部分 draft，等待 Task Merge Gate Review））；v0.5.0 尚未宣布完成；`v0.3.0` 全部完成（Chapter 01-07 最终完成，Part 02 收官）
...
1. **T01 input normalization（TASK-0032）**：**completed**（PR #60 squash merge `bfd1880`，分支已删）——Gate A（PR #59 `eb9d324`）→ Gate B/C（两轮 Review 修正 + 最终复审 APPROVED）→ Gate D（`docs/04-text2sql/ch19-input-normalization-intent.md`，draft，T01 部分）→ Task Merge Gate（两轮修正：field ownership → transition-scoped lifecycle authority）；`examples/text2sql_state/`：`state.py`（Text2SQLState）/ `normalization.py`（纯函数）/ `normalize_node.py`（Node adapter，success 只写 normalized_question、failure 仅 RUNNING→FAILED）+ 20 项测试。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（Gate E 属 Milestone Integration Closure，等 T02 进 main，非 T01 完成阻塞）；Chapter 19 = 不标 completed（T02 pending）；Part 04 = 进行中；v0.5.0 = 未完成。
2. **T03 metadata & business rule retrieval（TASK-0033，feature/t03-metadata-retrieval，Task Merge Gate Review 修正中）**：Gate A 通过（PR #59 合并 `eb9d324`）；Gate B 实现（`retrieval_types.py` RetrievalOutcome 五态 / RetrievalReference / MaterializedFacts / RetrievalResult / RetrievalCriteria（Proposed fixture）+ `metadata_source.py` InMemoryMetadataSource fake 权威源（exists/missing/ambiguous/unavailable/partial）+ `retrieval.py` MetadataRetriever（确定性聚合 UNAVAILABLE > AMBIGUOUS > NOT_FOUND > PARTIAL > COMPLETE）+ `retrieval_node.py` retrieve_metadata_node（只写 retrieval_result，**不触碰 shared lifecycle——Outcome ≠ Agent lifecycle**）+ `state.py` retrieval_result 字段（教学规模选择））；Gate C 测试通过（pytest / ruff / mkdocs --strict 全绿）；**Gate B/C 最终复审 APPROVED**（三轮修正：order-stability / UNAVAILABLE payload 策略 A / empty = contract violation / CatalogEntry 构造级 runtime 校验 / source index-entry identity 校验 / snapshot isolation）；**Gate D 完成**——`docs/04-text2sql/ch20-metadata-business-rule-retrieval.md`（draft，20.1-20.12，T03 完整承载；T04/Context Builder 仅接口位置）+ index/mkdocs 导航，content-map / ROADMAP 未动。**Task Merge Gate Review 修正（方案 A：fact-level provenance binding）**：`RetrievalReference.fact_id` + `MaterializedFact`（fact_id + content）——同一条 entry 共享稳定 fact_id，每条 materialized fact 可解析到自己的 provenance；6 项 binding tests。**Task Merge Gate 最终复审修正（identity / evidence 分离）**：新增 `CatalogEntry.entry_id`（稳定 source-local fact identity）；**fact_id 最终定义 = `f"{source.name}:{entry.entry_id}"`**（source-qualified entry identity，不再依赖 evidence / content）；source construction 校验 entry_id 全局唯一（**"Fact identity uniqueness is a source-boundary invariant."**）；固定原则 **"Fact identity ≠ freshness/version evidence."**（同一 entry_id evidence 更新仍同一事实；同 key 同 evidence 不同 entry_id = 不同事实；key = lookup identity、evidence = freshness 证据，均不承担 fact identity）；fixture entry_id（schema.orders / metric.gmv / region.east_china / metric.sales_definition.a/.b）；+4 项 identity/evidence 分离测试；Chapter 20 同步（20.4 Identity 模型四概念表 / 20.7 层 2 + identity chain / 20.9 / 20.12 / 验收）。**Task Merge Gate Architecture Review 已通过；最终 code-contract cleanup 已应用**：retrieval.py annotation 修正（list[str] → list[MaterializedFact]，ruff/pytest 通过 ≠ annotation 正确）；fact_id encoding grammar 冻结（source name / entry_id = non-empty / trimmed / 不含 ":"，source boundary ValueError fail-fast，不静默 normalize）；+8 项 grammar tests；Chapter 20 最小同步（20.4 encoding boundary 一句 + 20.9 evidence）。**状态快照**：Evidence = Contract-level verified（+ fact-to-reference provenance binding + fact identity uniqueness + fact/evidence separation + source-qualified identity string encoding 无歧义 verified；global uniqueness / cross-system identity / production URI scheme / production lineage ID scheme / production provenance schema 未验证）；Integration = **deferred**；等待 Merge 确认；Chapter 20 标 draft 不标 completed。
3. **T05 implementation task（SQL 静态校验，首个 implementation task，TASK-0030）**：**completed**（PR #56 合并 `b2572e6`）——Gate A（PR #55）→ Gate B/C → Gate D（Ch22 T05 部分）→ Task Merge Gate 全流程通过；`examples/text2sql_state/validation.py`（`_RULE_CHECKS` 无序 registry + RULE_ORDER 唯一顺序事实源 + `_statements` total contract + 超长 LIMIT 归入 limit_exceeds）；`docs/04-text2sql/ch22-sql-validation-repair-loop.md`（T05 可证实部分，T07 接口位置预留）。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（Gate E 等 T06/T07 进 main 后关闭）；Chapter 22 = 不标最终完成（T07 未实现）；Part 04 = 进行中；v0.5.0 = 未完成。**下一步按 TASK-0029 Recommended Implementation Waves 优先进入 T01 / T03（非立刻跳 T06/T07）**。
4. **Chapter 17 Ending maintenance（TASK-0027）已完成**：Chapter 17 Ending 句已修正为与冻结决策一致（"下一部分将进入 StateGraph 构图与 Graph Runtime 执行模型——图如何被组装、compile 如何将其转换为可执行 Runtime、invoke/stream 如何驱动执行，而不是重新定义这些运行时概念"），独立 PR 合并完成（commit b93f9a5）。
5. **Chapter 18 正文初稿（2026-08-08，TASK-0028，本任务；Part 04 前置章）**：
...
6. **未决项**：① Mermaid 计数漂移（历史 TASK / PR 记录与实际围栏数不一致；正文围栏数是事实源，后续任务不预先承诺固定数量）② v1.0.0 章节数目标对账（TASK-0014 未决项延续）③ 官方 URL 发布前复核（TASK-0014 未决项延续）④ RetryPolicy 机制归属（TASK-0014 未决项延续）
7. 补 tests/ 其余测试目标（State reducer、Tool adapter、Graph path、Checkpoint recovery）
8. 核验 Anthropic《Building effective agents》与 OpenAI practical guide 的官方 URL（第 0 章 TODO）
9. 选择许可证
10. **Future Task：LangChain Scope Planning**（不立即执行，仅记录方向；本条目已按 2026-08-05 规划合并补全，不新增第二份）：
...
11. **Future Backlog：Agent Workflow Patterns Scope Planning**（TASK-0031，proposed 登记，v0.6.0+，不立即执行）：

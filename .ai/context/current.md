...
1. **T05 implementation task（SQL 静态校验，首个 implementation task，TASK-0030）**：T01-T12 Execution Planning 完成（TASK-0029，PR #53）。**T05 实施顺序（TASK-0029 冻结）**：Gate A → Gate B Implementation → Gate C Tests/Evidence → Gate D Documentation → Task Merge Gate → Gate E Integration Closure（等 T06/T07 进 main 后，deferred → closed）。**当前阶段：Gate A APPROVED（PR #55）+ Gate B/C 完成并经最终复审修正**（text2sql_state 校验器：`_RULE_CHECKS` 无序 registry + RULE_ORDER 唯一顺序事实源 + `_statements` helper（separator-only SQL total contract 修复，";"/" ; ; " 安全返回 empty 不抛异常）；precedence 9 组 × 3 锁单测（不锁定 9 为不变量）；manual 回归对照 17 输入一致；**pytest 113 passed**）——**PR #56 待 Gate B/C 最终复审，通过后进入 Gate D（Ch22 T05 文档），再 Task Merge Gate**。T05 完成后按 Wave 推进其余 T01-T12（Chapter 19 起，按需使用 StateGraph API——选项 3 执行方式）。

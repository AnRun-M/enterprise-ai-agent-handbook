...
2. **T03 metadata & business rule retrieval（TASK-0033，feature/t03-metadata-retrieval，Gate D 完成待 Task Merge Review）**：Gate A 通过（PR #59 合并 `eb9d324`）；Gate B 实现（`retrieval_types.py` RetrievalOutcome 五态 / RetrievalReference / MaterializedFacts / RetrievalResult / RetrievalCriteria（Proposed fixture）+ `metadata_source.py` InMemoryMetadataSource fake 权威源（exists/missing/ambiguous/unavailable/partial）+ `retrieval.py` MetadataRetriever（确定性聚合 UNAVAILABLE > AMBIGUOUS > NOT_FOUND > PARTIAL > COMPLETE）+ `retrieval_node.py` retrieve_metadata_node（只写 retrieval_result，**不触碰 shared lifecycle——Outcome ≠ Agent lifecycle**）+ `state.py` retrieval_result 字段（教学规模选择））；Gate C 测试通过（pytest / ruff / mkdocs --strict 全绿）；**Gate B/C 最终复审 APPROVED**（三轮修正：order-stability / UNAVAILABLE payload 策略 A / empty = contract violation / CatalogEntry 构造级 runtime 校验 / source index-entry identity 校验 / snapshot isolation）；**Gate D 完成**——`docs/04-text2sql/ch20-metadata-business-rule-retrieval.md`（draft，20.1-20.12，T03 完整承载；T04/Context Builder 仅接口位置）+ index/mkdocs 导航，content-map / ROADMAP 未动。**状态快照**：Evidence = Contract-level verified；Integration = **deferred**（T02 未实现 / T04 未集成 / 真实 External Source 未接入）；等待 Task Merge Gate 最终 Review；Chapter 20 标 draft 不标 completed。

...
| Streaming | 无正文 | 无 | 输出流语义（可选留 Part 02） | Stream API | 生产传输协议 | 拆三处，不独立成章 | 拆三处 |
...
- **拆两处**：Checkpoint（Part 03 机制 + Part 05 生产恢复语义）、Interrupt（Part 03 原语 + Part 05 HITL）、Streaming（Runtime 输出流语义可选留 Part 02 / Stream API Part 03 / 传输协议 Part 05）

...
两个实现的等价测试 `test_direct_equivalence_with_manual` 断言的就是 State 字段：status / current_sql / execution_result / final_answer / iteration / history 动作序列——**行为等价 = 执行控制状态等价**。

TASK-0003 要求 `GraphState` 与 `AgentState` 字段语义对齐。跨轮次的执行控制信息全部在 State 里：
...
| 载体 | 承载什么 | 本项目对应 |
|---|---|---|
| **State** | 一次执行中的控制状态：轮次、生命周期、候选产物、校验结果、失败原因、history | `AgentState` / `GraphState` |
| **External Source of Truth** | 权限、元数据、语义层、业务规则、数据库 | canonical T03 / T06；`AGENTS.md` 安全底线 |
| **Checkpoint** | State 的持久化快照（断点续跑） | 未启用（v0.6.0 里程碑） |
| **Memory** | 跨任务或跨会话信息 | 未启用（v0.3.0 里程碑） |

**明确禁止：把所有外部业务事实复制进 State。** State 只承载执行控制状态：权限判定结果（准入/拒绝）可以进入 State，权限规则本体留在外部事实源；执行结果的控制信息（成功与否、行数）可以进入 State，完整业务数据留在数据库 / 结果集。
...
| `execution_result` | 工具输出的控制信息（T09，成功与否、行数），是 T12 最终回答的输入；完整业务数据不复制进 State |
